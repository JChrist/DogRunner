package jchrist.dogrunner.utils;
...
public class GifDecoder {
	...
	public Animation getAnimation(Animation.PlayMode playMode, float frameDuration) {
		...
		if(frameDuration < 0) {
			frameDuration = (float) getDelay(0);
	...
	public static Animation loadGIFAnimation(Animation.PlayMode playMode, InputStream is) {
		...
		gdec.read(is);
		return gdec.getAnimation(playMode, -1);
	}

	public static Animation loadGIFAnimation(Animation.PlayMode playMode, float frameDuration, InputStream is) {
		GifDecoder gdec = new GifDecoder();
		gdec.read(is);
		return gdec.getAnimation(playMode, frameDuration);
	}
}